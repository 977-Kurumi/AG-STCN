...
# from AGSTCN_TST_NOGLU import AGSTCN  #时空时原版卷积(加了注意力引导)
# from AGSTCN_ST_NOGLU import AGSTCN  #空时原版卷积(加了注意力引导)
# from AGSTCN_STT_NOGLU import AGSTCN  # 空时时原版卷积(加了注意力引导)
# from AGSTCN_STT_GLU import AGSTCN  #空时时  用了glu因果卷积(加了注意力引导)
# from AGSTCN_ST_GLU import AGSTCN  #空时   用了glu因果卷积(加了注意力引导)
# from AGSTCN_TST_GLU import AGSTCN  #时空时  用了glu因果卷积(加了注意力引导)
...
if __name__ == '__main__':
    ...
    #              num_timesteps_input,
    #              num_timesteps_output, dev=args.device).to(device=args.device)
    # net = STGCN(A_wave.shape[0],
    #              training_input.shape[3],
    #              num_timesteps_input,
    #              num_timesteps_output).to(device=args.device)
    net = GCN(A_wave.shape[0],
                 training_input.shape[3],
                 num_timesteps_input,
                 num_timesteps_output).to(device=args.device)
    # net = GLU(A_wave.shape[0],
    #              training_input.shape[3],
    #              num_timesteps_input,
    #              num_timesteps_output).to(device=args.device)
    # net = TGCN(training_input.shape[3], training_input.shape[1],
    #              64,
    #              2,
    #              num_timesteps_output, dev=args.device).to(device=args.device)
