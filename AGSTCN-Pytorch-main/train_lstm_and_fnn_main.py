...
from util.utils import generate_dataset, load_metr_la_data
...
if __name__ == '__main__':
    ...
    #
    net = LSTM(input_size=training_input.shape[3], hidden_size=hidden_size, num_layers=num_layers,
               output_size=num_timesteps_output).to(device=args.device)

    # net = FNN(input_size=training_input.shape[2], time_steps=training_input.shape[1], hidden_size1=32, hidden_size2=64,
    #           output_size=num_timesteps_output).to(device=args.device)
