...
if __name__ == '__main__':
    ...
    data = pd.read_csv('dym.csv', header=None)
    arr = np.array(data)

    tensor = torch.from_numpy(arr)
    tensor = tensor.reshape(1, 207, 12, 1)
    ...
    A_wave = A_wave.to(device=torch.device('cpu'))

    net = STGCN(A_wave.shape[0],
                tensor.shape[3],
                12,
                3).to(device=torch.device('cpu'))

    out = net(A_wave,tensor.to(device=torch.device('cpu')))
    #print(out.shape)
